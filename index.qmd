...
```{python}
#--testing function learned on DataCamp course--
def node_neigh(G,m):
  nodes= set()
...
The connections are pretty intense but I like the vibe. No labels needed because
the users are anonymous, so it serves us better as an overview of the clusters
that form.
...
users documented but it would be interesting to see what kind of topics are
associated with each node cluster.
...
the assignment, I'll include a few metrics.
